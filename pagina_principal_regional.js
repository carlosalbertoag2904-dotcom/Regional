(() => {
  ...
  // Normaliza nombre de columna (sin tildes, sin símbolos, minúsculas)
  ...
  function paintNameAndAsesoria() {
    ...
    // Si por alguna razón los slots no existieran, creamos cerca del icono
    if (!nameSlot || !aseSlot) {
  ...
  const AppYear = (() => {
    ...
    const setYear = (y) => {
      ...
      // Opcional: reescribe URL con ?year=YYYY para coherencia de navegación
      const url = new URL(location.href);
      url.searchParams.set("year", String(yy));
      // replace() para no dejar historial hacia adelante
  ...
  function mountYearTab() {
    ...
    list.addEventListener("click", (e) => {
      ...
      closePop();
      // Persistencia + navegación coherente
      AppYear.setYear(y);
    });
  }
...
/* ====== FIX: dropdowns estables por click (Nav y YearTab) ====== */
(function () {
  // --- A) Dropdown "Datos de la Empresa" ---
  document.addEventListener('click', (e) => {
    const trigger = e.target.closest('.nav-dropdown > .has-caret');
    const dd = e.target.closest('.nav-dropdown');

    // Abrir/cerrar si se hace click en el trigger
    if (trigger) {
      e.preventDefault();
      const parent = trigger.parentElement; // .nav-dropdown
  ...
  // Evita que el click dentro del menú lo cierre
  document.addEventListener('click', (e) => {
    const menu = e.target.closest('.nav-dropdown .dropdown-menu');
    if (menu) e.stopPropagation();
  }, true);

  // --- B) YearTab: robustecer el popover ---
  const host = document.getElementById('yearTab');
  if (host) {
    const pill = host.querySelector('.yt-pill');
    const pop  = host.querySelector('.yt-pop');
    if (pill && pop) {
      // Evita que el click dentro del pop cierre por el listener global
      pop.addEventListener('click', (ev) => ev.stopPropagation());
      pill.addEventListener('click', (ev) => ev.stopPropagation());
      // Cierre seguro al click global fuera
      document.addEventListener('click', (e) => {
