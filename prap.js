...
/* ============================
   PRAP (Administradores) — prap.js
   - Añade 'rid' por fila para enlazar avances
   - Persistencia por AÑO: YYYY_prap_<dirId>
============================ */
(function () {
  // ----- Año activo (URL > localStorage > año actual)
  ...
  function normalizeFila(f) {
    // Garantiza las claves esperadas y un rid estable
    return {
      rid: f.rid || rid(),
      obj: f.obj ?? "",
      ind: f.ind ?? "",
  ...
  // Solo re-render si cambia la clave del mismo AÑO e id
  window.addEventListener("storage", (e) => {
    if (e.key === KEY) render();
  });

  // Init: normaliza y re-escribe para añadir rid a datos viejos de ESTE AÑO
  escribir(leer());
  render();
})();
