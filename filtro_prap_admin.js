/* ============================
   Bloqueo de “volver hacia adelante”
   (no-forward history) — sólo en esta página
============================ */
(function () {
...
(function () {
  ...
  function renderTabla(items) {
    ...
    items.forEach(it => {
      ...
      // Abrir en la página ADMIN y preservar el año
      a.href = `prap.html?id=${encodeURIComponent(it.dirId)}&year=${YEAR}`;
      a.className = 'link-abrir';
      a.textContent = 'Abrir';

      // Reemplaza para evitar avanzar en historial
  ...
  // >>> FIX: al enfocar el input de Responsable, cerrar cualquier select abierto
  ...
  // Refrescar si otra pestaña cambia datos del mismo AÑO
...
/* ============================
   Dropdown header (click estable + ESC)
============================ */
(function(){
...
/* ============================
   Mejora visual del <select id="selDireccion">
   (se mantiene igual; sólo estilos en CSS)
============================ */
(function enhanceDireccionSelect(){
  ...
  // Inserta al lado del select y oculta el original
  ...
  // Cierre al hacer click fuera (también se cierra al enfocar el input de responsable por el fix)
  ...
  // Re-render cuando repoblas direcciones
