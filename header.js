/* =========================================================
   header.js — Guardia de sesión + Dropdown robusto (v4)
   - Evita doble binding
   - Ignora el click-fuera del mismo evento que abre
   - Soporta .open y .abierto
   ========================================================= */

/* --------- Guardia global de sesión + "Cerrar sesión" --------- */
(function () {
  ...
  const LOGIN_CANDIDATES = [
    ...
    "inicio.html",
    "inicio_regional.hmtl" // (typo heredado)
  ];
...
(function () {
  ...
  let lastToggleStamp = 0; // marca de tiempo del último toggle (para ignorar el click-fuera del mismo evento)
  ...
  // Abrir/cerrar clicando el trigger (en captura + corta propagación)
  document.addEventListener('click', (ev) => {
    const trigger = ev.target.closest(TRIGGER_SELECTOR);
    if (!trigger) return;
    const dd = trigger.closest(DD_SELECTOR);
    toggleDD(dd);
    lastToggleStamp = ev.timeStamp || Date.now(); // marca
    ev.preventDefault();
    ev.stopPropagation(); // evita que el "click fuera" del mismo evento cierre
  }, true);

  // Cerrar al clicar fuera (ignora el mismo evento que abrió)
  document.addEventListener('click', (ev) => {
    // si es el mismo evento (mismo timeStamp), no cierres
  ...
  // Teclado: Enter/Space activan en el trigger
