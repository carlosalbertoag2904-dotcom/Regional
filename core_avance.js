(function () {
  ...
  function migratePageRowsIfNeeded(ns, allRowsByRid) {
    // 1) mover page_rows_NS (sin prefijo) -> con año
    ...
    // 2) convertir claves simples rid -> "prapId::rid"
  ...
  /* ========= Deduplicación FUERTE por PRAP (en caliente) ========= */
  function fixDuplicateRidsForDir(dirId) {
    const rows = leerPrap(dirId);
    const seen = new Map(); // rid -> index
    let changed = false;
    ...
    rows.forEach((r, idx) => {
      ...
      if (!seen.has(rid)) {
        ...
        // Duplicado dentro del MISMO PRAP -> asigna rid nuevo y migra
        const oldRid = rid;
        const newR = newRid();
        rows[idx].rid = newR;
        changed = true;

        // 1) migrar aportes de todos los namespaces (prefijo de año y legacy)
        for (let i = 0; i < localStorage.length; i++) {
          const k = localStorage.key(i);
          if (!k) continue;

          // Solo claves del año actual o legacy prog_*
        ...
        // 2) migrar page_rows_* del año (a clave compuesta)
        for (let i = 0; i < localStorage.length; i++) {
          const k = localStorage.key(i);
  ...
  /* ========= Dataset PRAP del AÑO (con fix de duplicados) ========= */
  function loadAllRows() {
    const ids = leerDireccionesIds();
    // dedup fuerte en cada PRAP antes de cargar
  ...
  window.PrapAvanceNS.initAvancesPage = function ({ isAdmin, namespace }) {
    document.addEventListener("DOMContentLoaded", function () {
      ...
      // Migración page_rows legacy -> compuesta
    ...
    // Re-render SOLO para el año activo
